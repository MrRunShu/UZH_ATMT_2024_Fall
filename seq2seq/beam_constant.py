...
class BeamSearch(object):
    ...
    def prune(self):
        ...
        # Count finished nodes in the active queue
        active_finished = 0
        temp_nodes = []
        ...
        # Retain only the top (beam_size - active_finished) unfinished nodes
        unfinished_nodes = [
            (score, node) for score, node in temp_nodes if not node.finished
        ]
        unfinished_nodes.sort(key=lambda x: x[0])  # Sort by score (ascending)
        unfinished_nodes = unfinished_nodes[:self.beam_size - active_finished]

        # Add back all nodes (both finished and pruned unfinished) to the queue
        for score, node in temp_nodes:
            if node.finished or (score, node) in unfinished_nodes:
