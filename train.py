...
import random  # Import random module for BPE dropout
...
def get_args():
    ...
    # Add BPE dropout argument
    parser.add_argument('--bpe-dropout', default=0.0, type=float, help='BPE dropout rate (0.0 means no dropout)')
...
def apply_bpe_dropout(tokens, dropout_rate):
    """Applies BPE dropout by randomly dropping merges with a specified dropout rate."""
    if dropout_rate <= 0:
        return tokens
    dropped_sequences = []
    for sequence in tokens:  # Iterate over each sequence in the batch
        dropped_tokens = []
        for token in sequence:  # Process each token in the sequence
            token_str = str(token.item())  # Convert token to string for BPE split simulation
            if random.random() > dropout_rate:
                dropped_tokens.append(int(token_str))  # Keep token
            else:
                # Simulate token splitting; replace '_' if BPE separator is different
                split_tokens = [int(sub_token) for sub_token in token_str.split('_')]
                dropped_tokens.extend(split_tokens)
        dropped_sequences.append(dropped_tokens)
    return torch.tensor(dropped_sequences)  # Convert back to tensor format


def apply_bpe_dropout_to_sample(sample, dropout_rate):
    """Applies BPE dropout to each token in a sample if dropout is enabled."""
    if dropout_rate <= 0:
        return sample
    sample['src_tokens'] = apply_bpe_dropout(sample['src_tokens'], dropout_rate)
    return sample
...
def main(args):
    ...
    # Load datasets with BPE dropout applied if specified
    def load_data(split):
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            if len(sample) == 0:
                continue
            model.train()
            # Apply BPE dropout to the sample tokens if enabled
            if args.bpe_dropout > 0.0:
                sample = apply_bpe_dropout_to_sample(sample, args.bpe_dropout)
